...
LICENSE="LGPL-2+"
SLOT="3"
IUSE="aqua broadway cloudprint colord cups examples gtk-doc +introspection test usr-doc vim-syntax wayland X xinerama"
REQUIRED_USE="
	|| ( aqua wayland X )
	xinerama? ( X )
"
...
#	if ! use gtk-doc ; then
#		# don't waste time building tests
#		strip_builddir SUBDIRS docs Makefile.{am,in}
#	fi
...
pkg_postrm() {
	...
	if [[ -z ${REPLACED_BY_VERSIONS} ]]; then
		multilib_pkg_postrm() {
			rm -rf "${EROOT}"usr/$(get_libdir)/gtk-3.0/3.0.0/immodules.cache
		}
